...
class Cart
{
	...
	public function addToCart($quantity, $id)
	{
		...
		if ($getPro) {
			...
			if ($inserted_row) {
				...
				echo '<script>window.location="details.php?proid=' . $id . '";
	...
	WHERE cartId = '$cartId'";
		...
		if ($updated_row) {
			...
			$sum = Session::get("sum");
			$qty = Session::get("qty");
			// header("Refresh:0; url=inc/header.php");
	...
	public function orderProduct($cmrId)
	{
		...
		if ($getPro) {
			while ($result = $getPro->fetch_assoc()) {
				...
				$prd = new Product();
				$prd = $prd->getpd($productId);
				$rs = $prd->fetch_assoc();
				// die($rs['quantity']);
				$qt = $rs['quantity'] - $quantity;
				$up = "UPDATE tbl_product set quantity = $qt where productId = $productId";
				$this->db->update($up);
